...
void readArgs(int argc, char **argv)
{
    ...
    else
    {
        std::cerr << "Error: unknown command '" << cmd
                  << "'. Expected one of: superbubbles, directed-superbubbles, snarls, spqr-tree.\n\n";
        usage(args[0].c_str(), 1);
    }
...
namespace solver
{
    ...
    namespace snarls
    {
        ...
        void output_spqr_tree_only()
        {
            ...
            // std::cout << "Writing connected components..." << std::endl;
            for (int ccIdx = 0; ccIdx < numCC; ++ccIdx)
            {
                // std::cout << " Processing component " << ccIdx << " with "
                //           << ccNodes[ccIdx].size() << " nodes." << std::endl;
                ...
                else
                {
                    ...
                    // std::cout << " Computing biconnected components..." << std::endl;
                    ...
                    // Iterate through graph nodes and find cut vertices
                    // std::cout << " Writing cut vertices..." << std::endl;
                    for (ogdf::node v : ccGraph.nodes)
                    {
                        assert(v != nullptr);
                        // std::cout << "  Checking node " << v->index() << "..." << std::endl;
                        if (bc.typeOfGNode(v) == ogdf::BCTree::GNodeType::CutVertex)
                        {
                            // std::cout << "  is cut vertex" << std::endl;
                            ogdf::node origNode = ccToOrig[v];
                            // std::cout << "  has original node " << origNode->index() << std::endl;
                            out << "C " << C.node2name[origNode];

                            // Find all blocks containing this cut node
                            ogdf::node vBC = bc.bcproper(v);
                            // std::cout << "  has bcproper node " << vBC->index() << std::endl;
                    ...
                    // Compute SPQR tree for each block
                    for (ogdf::node bNode : bc.bcTree().nodes)
                    {
                        if (bc.typeOfBNode(bNode) != ogdf::BCTree::BNodeType::BComp)
                        {
                        ...
                        // std::cout << "  Computing SPQR for block " << bNode->index() << "..." << std::endl;

                        std::string blockName = bcNodeToBlockName[bNode];

                        // Create subgraph for this block
                        ...
                        try
                        {
                            ...
                            for (ogdf::edge treeEdge : spqr.tree().edges)
                            {
                                ...
                                // Get the virtual edge endpoints
                                // In OGDF SPQR tree, tree edges correspond to virtual edges in skeletons
                                const ogdf::Skeleton &skelSrc = spqr.skeleton(src);

                                // Find the virtual edge in the skeleton
